        ...
        <ShareDateView_Container>
                ...
                <SideScrollBox_Container>
                    {friendListState.data && friendListState.data.map((friend) => (
                        <MapProfile_Container onClick={() => setTargetHandler("friendMsg", friend.chattingRoomId)}>
                            ...
                            <ProfileContainer $userId={friend.friendUserId}
                                              style={{width: '30px', height: '30px', boxSizing: 'border-box'}}/>
                            <ProfileName_Wrapper>
                                {friend.nickName}
                            </ProfileName_Wrapper>
                        </MapProfile_Container>
            ...
            <ShareButton_Container>
                <RowFlexBox>
                    ...
                    {isHover &&
                        <InfoBox text={`일정을 공유받은 대상이 해당 일정을 복사할 수 있는여부 (현재상태: ${copyAble ? "복사 가능" : "복사 불가능"})`}
                                 marginLeft={-10} marginTop={-60}/>}
                    <div style={{fontSize: '10px', marginLeft: '-5px'}}>
                        일정 복사 가능여부
                    </div>
                    <Toggle_Container onClick={setCopyAble} $toggleState={copyAble}
