            ...
            <EventFriendListMiddle_Container>
                {(friendListState.data && friendListState.data.length>0) ?
                    (friendListState.data.map((friend)=>(
                        <EventFriendItem_Container key ={friend.friendUserId} onClick={()=>{friendListHandler(friend.friendUserId)}}>
                            <EventFriendName_Container>
                                {friend.nickName}
                            </EventFriendName_Container>
                            <FriendJoinState_Button $isJoin={eventState.friendList.some((id)=>id===friend.friendUserId)}>
                                {eventState.friendList.some((id)=>id===friend.friendUserId) ?'제거':'추가'}
                            </FriendJoinState_Button>
                        </EventFriendItem_Container>
                    )))
                    :
                    <div style={{width:'100%',height:'100%', display:'flex', alignItems:"center", justifyContent:'center', fontSize:'13px', color:`${TextColor}77`}}>
                        친구가 존재하지 않습니다.
                    </div>
                }
            </EventFriendListMiddle_Container>
