...
export const LocationSetModal :React.FC<ScheduleDetailProps> =({dispatchSubSchedule,subScheduleIndex, onClose})=>{
    ...
    const setSubScheduleLocation = ()=>{
        ...
        const setLocation =()=>{
            dispatchSubSchedule({type:'SET_LOCATION', payload:{index:subScheduleIndex, location:locateItem.address}})
            ...
            <LocationSet_Container onSubmit={findByKeyword} $isPick={locateItem!==null}>
                ...
                {addressList.length>0 &&
                    <SearchKeyword_Container>
                        {addressList.map((address)=>(
                            ...
                            <SearchKeyword_Item onClick={()=>setSearchKeyword(address.jibunAddress)} key={address.distance}>
                ...
                <SearchResult_Container>
                    {locatePosition===null ?
                        ...
                        <SearchResultItem_Wrapper key={locate.mapx} onClick={()=>locatehandler(locate)}>
                            ...
                            <div style={{width:`calc(100% - 26px)`, textOverflow:'ellipsis', overflow:'hidden', whiteSpace:'nowrap'}} dangerouslySetInnerHTML={{__html: locate.title}}/>
                            </SearchResult_Title>
                            <SearchResult_Category>
                                {locate.category}
                            </SearchResult_Category>
                ...
                {locateItem &&
                    <LocateButton_Container>
                        ...
                        <Modal_Condition_Button style={{height:'40px'}} $isAble={true} onClick={setSubScheduleLocation}>
                            지정하기
                        </Modal_Condition_Button>
                    </LocateButton_Container>
                }
