...
import {promiseDebouncing} from "./promiseDebouncing";
import {checkEmailValidApi} from "../api/checkEmailValidApi"
import {SignFormData} from "./types"

export const schema: yup.ObjectSchema<SignFormData> = yup.object().shape({
    nickname: yup.string()
    ...
    emailValidation : yup.boolean()
        .test('이메일 인증 스키마','이메일 인증을 해주세요',  (value)=>{
            ...
            return promiseDebouncing(checkEmailValidApi)
                .then((resolve)=>{
                    return resolve
                })
                .catch((err)=>{
