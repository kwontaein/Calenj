...
const MakeVote :React.FC<ModalProps>=({onClose, groupId})=>{
    ...
    return(
        <div>
            ...
            <RowFlexBox>
                <Mini_Input onChange={(e:ChangeEvent<HTMLInputElement>)=>setTitle(e.target.value)} ref={inputRef} style={{marginTop:'1px', width:'210px', fontWeight:'550'}} placeholder='투표 제목'/>
            </RowFlexBox>
            ...
            <label style={{fontSize:'13px'}}><input type='radio' name='inputForm' value='TEXT' onClick={(e)=>inputFormHandler(e)} defaultChecked/>텍스트</label>
            <label style={{fontSize:'13px'}}><input type='radio' name='inputForm' value='DATE' onClick={(e)=>inputFormHandler(e)}/>날짜</label>
            ...
            <DatePicker
                ...
                locale={ko}
             />
            <label style={{fontSize:'13px',marginLeft:'3px'}}><input type='checkBox' name='choice' onChange={(e:React.ChangeEvent<HTMLInputElement>)=>{setMultipleOption(e.target.checked)}}/>복수선택</label>
            <label style={{fontSize:'13px'}}><input type='checkBox' name='anonymous' onChange={(e:React.ChangeEvent<HTMLInputElement>)=>{setanonymousOption(e.target.checked)}}/>익명투표</label>
