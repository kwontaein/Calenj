...
public interface FriendRepository extends JpaRepository<FriendEntity, FriendId> {
    @Query("SELECT new org.example.calenj.Main.DTO.User.FriendResponse(f.friendUserId,f.nickName,f.ChattingRoomId,f.createDate) FROM Friends f WHERE f.ownUserId.userEmail =:userId and f.status =ACCEPT")
    Optional<List<FriendResponse>> findFriendListById(@Param("userId") String userId);

    @Query("SELECT new org.example.calenj.Main.DTO.User.FriendResponse(f.friendUserId,f.nickName,f.ChattingRoomId) FROM Friends f WHERE f.ownUserId.userEmail =:userId and f.status =ACCEPT")
