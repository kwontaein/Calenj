...
public interface Group_NoticeRepository extends JpaRepository<GroupNoticeEntity, GroupNoticeId> {
    ...
    @Query("SELECT new org.example.calenj.Main.DTO.Response.Group.GroupNoticeResponse(gn.noticeId, gn.noticeContent, gn.noticeCreater, gn.noticeCreated) FROM Group_Notice gn WHERE gn.group.groupId = :groupId")
    ...
    @Query("SELECT new org.example.calenj.Main.DTO.Response.Group.GroupNoticeResponse(gn.noticeId, gn.noticeContent, gn.noticeCreater, gn.noticeCreated, gn.noticeWatcher) FROM Group_Notice gn WHERE gn.noticeId = :noticeId")
