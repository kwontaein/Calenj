...
public interface GroupRepository extends JpaRepository<GroupEntity, UUID> {
    ...
    @Query("select new org.example.calenj.Main.DTO.Group.GroupDTO.Response(g.groupId, g.groupTitle) from Group_table g JOIN Group_User gu ON g.groupId = gu.group.groupId where gu.user.userEmail = :userEmail")
    Optional<List<GroupDTO.Response>> findByUserEntity_UserEmail(@Param("userEmail") String userEmail); // No argument for named parameter ':groupCreater'
    ...
    @Query("SELECT new org.example.calenj.Main.DTO.Group.GroupDTO.Response(g.groupId, g.groupTitle, g.groupCreated ,g.groupCreater) FROM Group_table g WHERE g.groupId = :groupId")
    Optional<GroupDTO.Response> findGroupById(@Param("groupId") UUID groupId);
