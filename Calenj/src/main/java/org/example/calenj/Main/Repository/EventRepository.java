...
public interface EventRepository extends JpaRepository<EventEntity, EventId> {
    ...
    //모든 이벤트 리스트
    @Query("SELECT new org.example.calenj.Main.DTO.Response.User.EventResponse" +
    ...
    //요청한 친구 추가 이벤트
    @Query("SELECT new org.example.calenj.Main.DTO.Response.User.EventResponse(e.eventId,e.ownUserId.userEmail,e.eventUserId,e.eventPurpose,e.eventName,e.eventStatus,e.createDate) FROM Events e WHERE e.ownUserId.userEmail =:userId and e.eventName=RequestFriend and e.eventStatus=2")
    Optional<List<EventResponse>> RequestEventListById(@Param("userId") String userId);

    //요청받은 친구 추가 이벤트
    @Query("SELECT new org.example.calenj.Main.DTO.Response.User.EventResponse(e.eventId,e.ownUserId.userEmail,e.eventUserId,e.eventPurpose,e.eventName,e.eventStatus,e.createDate) FROM Events e WHERE e.eventUserId =:userId and e.eventName=RequestFriend and e.eventStatus=2")
    Optional<List<EventResponse>> ResponseEventListById(@Param("userId") String userId);

    @Query("SELECT new org.example.calenj.Main.DTO.Response.User.EventResponse(e.eventId,e.ownUserId.userEmail,e.eventUserId,e.eventPurpose,e.eventName,e.eventStatus,e.createDate) FROM Events e WHERE e.ownUserId.userEmail =:ownUserId and e.eventUserId =:friendId and e.eventName=RequestFriend")
