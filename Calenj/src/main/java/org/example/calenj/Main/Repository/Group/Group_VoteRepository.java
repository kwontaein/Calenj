...
public interface Group_VoteRepository extends JpaRepository<GroupVoteEntity, GroupVoteId> {
    ...
    @Query("SELECT new org.example.calenj.Main.DTO.Response.Group.GroupVoteResponse(gv.voteId, gv.voteCreater, gv.voteTitle, gv.voteCreated, gv.voteEndDate, gv.countVoter) FROM Group_Vote gv WHERE gv.group.groupId = :groupId")
    ...
    //voteId로 상세조회
    @Query("SELECT new org.example.calenj.Main.DTO.Response.Group.GroupVoteResponse(gv.voteCreater, gv.voteTitle, gv.voteCreated, gv.voteEndDate,gv.isMultiple, gv.anonymous, gv.voteWatcher,gv.countVoter) FROM Group_Vote gv WHERE gv.voteId = :voteId")
