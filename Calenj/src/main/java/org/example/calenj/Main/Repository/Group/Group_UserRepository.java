...
public interface Group_UserRepository extends JpaRepository<GroupUserEntity, GroupUserId> {
    ...
    @Query("SELECT new org.example.calenj.Main.DTO.Group.GroupUserDTO.Response(gu.user.userEmail,gu.user.nickname,gu.user.isOnline, gu.role, gu.group_user_location) FROM Group_User gu WHERE gu.group.groupId = :groupId")
    List<GroupUserDTO.Response> findGroupUsers(@Param("groupId") UUID groupId);
