...
public interface GroupRepository extends JpaRepository<GroupEntity, UUID> {
    ...
    @Query("select new org.example.calenj.group.groupinfo.dto.response.GroupResponse(g.groupId, g.groupTitle) from Group_table g JOIN Group_User gu ON g.groupId = gu.group.groupId where gu.user.userEmail = :userEmail")
    Optional<List<GroupResponse>> findByUserEntity_UserEmail(@Param("userEmail") String userEmail); // No argument for named parameter ':groupCreator'
